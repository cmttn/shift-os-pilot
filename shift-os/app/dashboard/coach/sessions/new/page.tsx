...
export default async function NewSessionPage({ searchParams }: NewSessionPageProps) {
  ...
  const friendlyOnly = searchParams?.mode === 'friendly';
  const writableTeams = friendlyOnly
    ? coachData.teams
    : coachData.teams.filter((team) => !team.is_club_managed || team.allow_coach_fixture_imports);
  if (coachData.teams.length > 0 && writableTeams.length === 0) redirect('/dashboard/coach');
  const writableCoachData = { ...coachData, teams: writableTeams, activeTeamId: writableTeams[0]?.id ?? '' };
  ...
  return (
    <main className="min-h-screen px-5 pb-32 pt-8 text-white" style={{ backgroundColor: '#080a0f' }}>
      <div className="mx-auto max-w-[480px]">
        <h1 className="text-3xl font-black tracking-tight">{friendlyOnly ? 'Add Friendly' : 'Add Session'}</h1>
        <p className="mt-2 text-sm text-white/40">{friendlyOnly ? 'Create a manual friendly fixture for your team.' : 'Create a match, training session or tournament.'}</p>
        <section className="mt-6 rounded-2xl border p-5" style={{ background: 'linear-gradient(145deg,#0d1117,#0a0e15)', borderColor: 'rgba(255,255,255,0.06)' }}>
          <CreateSessionForm coachData={writableCoachData} initialType={friendlyOnly ? 'match' : initialType} friendlyOnly={friendlyOnly} />
        </section>
      </div>
    </main>
  );
