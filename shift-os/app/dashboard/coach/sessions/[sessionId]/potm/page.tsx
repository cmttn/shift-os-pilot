...
export default function SessionPotmPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [{ data: teamData }, { data: pollData }] = await Promise.all([
      ...
      supabase.from('potm_polls').select('id,session_id,team_id,status,poll_opens_at,poll_closes_at,winner_player_id,total_votes,social_card_url').eq('session_id', sessionId).maybeSingle<PotmPollRow>()
    ]);
      ...
      <div className="mx-auto max-w-[760px]">
        ...
        {!poll || poll.status === 'cancelled' ? (
          <section className="mt-6 rounded-2xl border p-6" style={{ background: 'linear-gradient(145deg,#0d1117,#0a0e15)', borderColor: 'rgba(255,255,255,0.06)' }}>
            ...
            <div className="mt-5 rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
              ...
              <p className="mt-2 text-sm italic text-white/70">“{message}”</p>
        ...
        {poll?.status === 'open' ? (
          <section className="mt-6 rounded-2xl border border-amber-400/25 bg-amber-400/10 p-6">
            ...
            <button type="button" onClick={closePoll} className="mt-5 w-full rounded-full px-6 py-4 font-semibold text-black" style={{ backgroundColor: primaryColour }}>Close Poll & Announce Winner</button>
          </section>
        ) : null}

        {poll?.status === 'closed' ? (
          <section className="mt-6 rounded-2xl border p-6 text-center" style={{ background: 'linear-gradient(145deg,#0d1117,#0a0e15)', borderColor: 'rgba(245,158,11,0.4)', boxShadow: '0 20px 60px rgba(245,158,11,0.08)' }}>
            ...
            {poll.social_card_url ? <img src={poll.social_card_url} alt="" className="mt-5 w-full rounded-2xl border border-white/10" /> : null}
            <button type="button" onClick={closePoll} className="mt-5 rounded-full border border-white/10 px-4 py-2 text-sm text-white">Regenerate Card</button>
          </section>
        ) : null}
