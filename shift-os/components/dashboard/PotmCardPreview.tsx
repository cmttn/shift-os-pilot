...
export default function PotmCardPreview({ playerName, teamName, opponent, message, primaryColour, secondaryColour, badgeUrl, clubName }: PotmCardPreviewProps) {
  ...
  return (
    <article className="relative aspect-square w-full overflow-hidden rounded-[28px] border border-white/10 shadow-2xl" style={{ backgroundColor: deep }}>
      ...
      <svg className="absolute right-[-10%] top-[24%] h-[52%] w-[48%] text-white/[0.035]" viewBox="0 0 180 220" aria-hidden="true">
        <path
          fill="currentColor"
      ...
      <div className="relative flex h-full flex-col p-5 sm:p-8">
        <div className="relative h-[29%] min-h-[106px]">
          ...
          <div className="absolute left-0 top-9 flex h-28 w-28 items-center justify-center drop-shadow-[0_18px_32px_rgba(0,0,0,0.45)] sm:h-40 sm:w-40">
        ...
        <div className="flex min-h-0 flex-1 flex-col justify-center py-2">
          <h3 className="text-center text-[clamp(32px,8.6vw,60px)] font-black leading-[0.98] tracking-tight text-white drop-shadow-[0_8px_20px_rgba(0,0,0,0.35)] [overflow-wrap:anywhere]">{playerName}</h3>
          <div className="mt-3 text-center sm:mt-4">
            <p className="text-base font-bold text-white/90">{teamName}</p>
            <p className="mt-0.5 text-sm text-white/60">vs {opponent}</p>
          </div>
          <p className="mx-auto mt-3 inline-flex rounded-full border border-white/10 bg-black/10 px-2.5 py-1 text-[9px] font-semibold uppercase tracking-[0.13em] text-white/55 sm:text-[10px]">Awarded by parents</p>
        </div>

        <div className="rounded-2xl border border-white/10 bg-black/25 p-3 backdrop-blur sm:p-4">
        ...
        <div className="flex items-center justify-between gap-4 pt-3 text-[9px] font-semibold uppercase tracking-[0.18em] text-white/38 sm:pt-4 sm:text-[10px]">
