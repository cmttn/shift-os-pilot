...
type MutableSession = ParentSession;
...
export default function ParentFixturesClient({ playerId, playerName, team, heroSessionId }: ParentFixturesClientProps) {
  const [sessions, setSessions] = useState<MutableSession[]>(team.upcoming_sessions);
  ...
  return (
    <>
      {heroSession ? (
        <section className="relative mt-8 overflow-hidden rounded-2xl p-5" style={{ background: `linear-gradient(135deg, ${team.club_primary_colour} 0%, #102015 52%, #080a0f 100%)` }}>
          ...
          <div className="relative">
            ...
            <h2 className="mt-3 text-2xl font-black text-white">{sessionTitle(team.team_name, heroSession)}</h2>
            ...
            <div className="mt-6">
              {heroSession.my_availability === 'week_off' ? (
                <div className="flex min-h-14 items-center justify-center rounded-full bg-white/10 px-4 text-sm font-semibold text-white/65">🏖️ Week Off — set by coach</div>
      ...
      <div className="mt-4 space-y-3">
        {visibleSessions.length === 0 ? (
          ...
          return (
            <article key={session.id} className="rounded-xl border p-4" style={{ backgroundColor: 'rgba(255,255,255,0.03)', borderColor: 'rgba(255,255,255,0.06)', borderLeft: `3px solid ${accent}` }}>
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs text-white/30">{session.location ?? session.full_address ?? 'Location TBC'}</p>
